    ...
    <form className="sidebar-form" onSubmit={handleSubmit}>
      ...
      <div className="combobox">
        ...
        <input
          ...
          value={name}
          onChange={(event) => { setName(event.target.value); setChosenName(null); }}
        ...
        <p className="field-hint">
          {searchError ? <span className="form-error-inline">{searchError}</span>
            : searching ? "Searching…"
            : chosenName === name.trim() ? "Name, address and location taken from this place."
      ...
      <div className="location-block">
        <span className="location-status">
          {location
            ? <>Location set · <span className="muted">{location.lat.toFixed(5)}, {location.lng.toFixed(5)}</span></>
            : "No location yet"}
        </span>
