...
export default function Sidebar(props: SidebarProps) {
  ...
  return <aside className="sidebar">
    ...
    <div className="sidebar-body">
      {showDetail ? <PlaceDetail selectedPlace={props.selectedPlace} selectedPlaceLoading={props.selectedPlaceLoading} onSelectPlace={selectPlace} onSubmitReview={props.onSubmitReview} onUpdateReview={props.onUpdateReview} /> : mode === "add" ? <AddPlaceForm onSubmit={async (input, password) => { await props.onAddPlace(input, password); setMode("list"); }} onCancel={() => setMode("list")} pickMode={props.pickMode} onPickModeChange={props.onPickModeChange} pickedLocation={props.pickedLocation} /> : props.placesError ? <p className="form-error" role="alert">{props.placesError}</p> : <><PlaceList places={props.places} placesLoading={props.placesLoading} query={props.query} onSelectPlace={selectPlace} /><button type="button" className="primary-button add-place-button" onClick={() => setMode("add")}>Add a place</button></>}
    </div>
  </aside>;
}
